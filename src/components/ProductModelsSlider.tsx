  ...
  return <div className={`group relative bg-card rounded-2xl shadow-lg transition-all duration-500 ease-in-out transform-gpu ${isHovered ? 'scale-105 shadow-2xl shadow-primary/30 z-50' : 'hover:shadow-xl hover:shadow-primary/20 z-10'}`} style={{
    ...
    overflow: isHovered ? 'visible' : 'hidden'
      ...
      <div className="p-6">
        ...
        <Button className="hidden md:block mt-4 w-full group/btn bg-primary hover:bg-primary/90 text-primary-foreground rounded-full transition-all duration-300 flex items-center justify-center" onClick={() => window.location.href = model.link}>
          Ver Mais
          <ArrowRight className="w-4 h-4 ml-2 transition-transform group-hover/btn:translate-x-1" />
        </Button>
      </div>
